...
@injectable
class HostCreateController extends ChangeNotifier {
  final HostService hostService;
  final KeyService keyService;

  HostCreateController(this.hostService, this.keyService);
  ...
  bool get isCreatingKey => _isCreatingKey;
  ...
  Future<void> createInlineKey({
    ...
    notifyListeners();
    try {
      final key = await keyService.createKey(
        label: label,
        value: value,
  ...
  Future<void> createHost(BuildContext context) async {
    _isLoading = true;
    _error = null;
    notifyListeners();
    try {
